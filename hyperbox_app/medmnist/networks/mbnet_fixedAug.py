...
class DAMobile3DNetFixedAug(BaseNASNetwork):
    def __init__(
        self,
        in_channels=3, 
        input_channel=None,
        ...
        super(DAMobile3DNetFixedAug, self).__init__(mask)
        ...
        self.augmentation = DataAugmentation(
            ...
            mean, std, aug_keys, ignore_keys, aug_mask)
...
if __name__ == '__main__':
    ...
    net = DAMobile3DNetFixedAug(1, num_classes=10,
        aug_mask='/home/xihe/xinhe/hyperbox_app/logs/runs/hyperbox_project/adrenal3d_daresnet/mask_json/mask_epoch_12.json').to(device)
    dm = OnehotMutator(net)
    for i in range(10):
        dm.reset()
        # if i > 5:
        #     net = net.eval()
        x = torch.rand(2,1,16,64,64).to(device)
        y = net(x)
        print(y.argmax(-1))
