...
if __name__ == "__main__":
    ...
    for cell in net.cells:
        hook = cell.register_full_backward_hook(backward_hook)
    ...
    for i in range(3):
        ...
        z = (y-torch.rand(num, 10).to(device)).sum()
        z.backward()
    ...
    #     ID1 = twonn1(feat.view(num, -1).cpu().detach().numpy(), False)
    #     ID2 = twonn2(feat.view(num, -1).cpu().detach().numpy(), False)
    #     # ID3 = skdim.id.ESS().fit_transform(X=feat.detach().cpu().numpy())
    #     ID3 = 0
    #     print(idx, ID1, ID2, ID3)
    #     ID1s.append(ID1)
    #     ID2s.append(ID2)
    # for i, IDs in enumerate([ID1s, ID2s]):
    #     model_ids[i] = {}
    #     model_ids[i]['label'] = f"{model_name}_twonn{i+1}"
    #     model_ids[i]['IDs'] = ID1s
