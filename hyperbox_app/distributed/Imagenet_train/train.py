...
from timm.models.ofa_model import OFAMobileNetV3
...
sys.path.append('/home/xihe/xinhe/GM-NAS//ofa')
sys.path.append('/home/comp/18481086/code/hyperbox_app/hyperbox_app')
...
parser.add_argument('--data', metavar='DIR',
                    help='path to dataset', default='/home/xihe/datasets/imagenet2012')
parser.add_argument('--model', default='resnet101', type=str, metavar='MODEL',
                    help='Name of model to train (default: "countception"')
parser.add_argument('--evalonly', type=int, default=0, help='0: training; 1: only evaluation')
parser.add_argument('--dist_url', default='tcp://127.0.0.1:8888', type=str,
...
parser.add_argument('--patience-epochs', type=int, default=20, metavar='N',
                    help='patience epochs for Plateau LR scheduler (default: 20')
...
# knowledge dstill
parser.add_argument('--knowledge_distill', action='store_true', default=False,
                    help='use the teacher to guide the neural network training')
parser.add_argument('--kd_ratio', type=float, default=0.0,
                    help='the ratio between knowledge distill loss and the original loss')
...
def main():
    ...
    if args.model in model_names:
        model_cfg = arch_maps[args.model]
        ...
        if args.model == 'ofa':
            ...
            model_cfg.depth_list = [3,4]
            model_cfg.expand_ratio_list = [4,6]
            ...
            # tmp_mask_path = os.path.join(args.output, 'mask.json')
    ...
    if args.knowledge_distill:
        if args.teacher_name == 'D-Net-big224':
            args.teacher_model = create_model(
                'D-Net-big224',
                pretrained=args.pretrained,
                num_classes=args.num_classes,
                drop_rate=args.drop,
    ...
    if args.local_rank == 0:
        ...
        with open(os.path.join(output_dir, 'args.yaml'), 'w') as f:
            f.write(args_text)
        os.system(f"cp {args.mask} {os.path.join(output_dir, 'arch.json')}")
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if args.teacher_model is not None:
            ...
            if args.img_size_teacher != args.img_size:
                input_teacher = F.interpolate(input, size = [args.img_size_teacher, args.img_size_teacher], mode='bicubic')
            else:
                input_teacher = input
            with torch.no_grad():
                soft_logits = args.teacher_model(input_teacher).detach()
                soft_label = F.softmax(soft_logits, dim=1)

        # output, aux_output = model(input)
        output = model(input)

        if args.kd_ratio == 0 or epoch >= args.teacher_off_epoch:
        ...
        # loss_aux = loss_fn(aux_output, target)
        # loss += 0.4 * loss_aux
...
# bash /home/comp/18481086/code/GM-NAS/Imagenet_train/distributed_train.sh 4 \
# /home/xihe/datasets/imagenet2012 --model proxylessnas --opt rmsproptf --opt-eps .001 --lr 0.001 \
# --output proxyless-pretrained \
# --weights /home/comp/18481086/code/hyperbox_app/hyperbox_app/distributed/networks/proxylessnas/proxyless.pth \
# --mask /home/comp/18481086/code/hyperbox_app/hyperbox_app/distributed/networks/proxylessnas/proxyless_2.json -b 128

#  bash /home/comp/18481086/code/GM-NAS/Imagenet_train/distributed_train.sh 4 \
# /home/xihe/datasets/imagenet2012 --opt rmsproptf --opt-eps .001 --model proxylessnas \
# --mask /home/comp/18481086/codepp/hyperbox_app/distributed/finetune_masks/proxyless_masks/ID/v1/0_0.6813.json  \
# --output proxyless-v10 -b 128 \
# --weights /home/comp/18481086/code/hyperbox_app/hyperbox_app/distributed/networks/proxylessnas/proxyless.pth 


# bash /home/comp/18481086/code/GM-NAS/Imagenet_train/distributed_train.sh 4 /home/xihe/datasets/imagenet2012 --opt rmsproptf --opt-eps .001 --model proxylessnas --mask /home/comp/18481086/code/hyperbox_app/hyperbox_app/distributed/finetune_masks/proxyless_masks/ID/v1/0_0.6813.json  --output proxyless-v10-kd -b 64 --weights /home/comp/18481086/code/hyperbox_app/hyperbox_app/distributed/networks/proxylessnas/proxyless.pth --lr 0.0025 --knowledge_distill --kd_ratio 1.0 --teacher_name resnet50

# bash /home/xihe/xinhe/GM-NAS/Imagenet_train/distributed_train.sh 4 --data /home/xihe/datasets/imagenet2012 --model ofa --opt sgd  --lr 0.001 --output ofa_1_mixup -b 256 --mask /home/xihe/xinhe/hyperbox_app/hyperbox_app/distributed/logs/runs/ea_search_ofa_ea/ea_search_ofa/evolution_logs/1_0.78164.json --mixup 0.2 --mixup-off-epoch 250